...
**/
    ...
    tStackP stackP;
    SInitP(&stackP);
	tBTNodePtr tmpRoot = RootPtr;
	
	Leftmost_Postorder (tmpRoot, &stackP, &stackB);
	
	while(SEmptyP(&stackP) != TRUE)
	{
		tmpRoot = STopPopP(&stackP);
		SPushP(&stackP,tmpRoot);
		
		if(STopPopB(&stackB))
		{
			SPushB(&stackB,FALSE);
			Leftmost_Postorder(tmpRoot->RPtr, &stackP, &stackB);
		}
		else
		{
			STopPopP(&stackP);
...
**/
	...
	while(1)
	{
		...
		if((SEmptyP(&zasobnik)) && ((*RootPtr) == NULL))
            return;
        else 
			continue;
	}		
